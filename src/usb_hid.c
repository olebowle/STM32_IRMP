...
ErrorStatus USB_HID_SendData(uint8_t *ptr, uint8_t len) // len: bytes to send (1...HID_IN_BUFFER_SIZE)
{
  uint8_t n;
  ...
  USB_HID_IN_BUF[0]=0x04; // Report ID

  for(n=0;n<=HID_IN_BUFFER_SIZE;n++) {
    if(n<=len) {
      USB_HID_IN_BUF[n+1]=*ptr; // nach Report ID
      ptr++;
    }
    else {
...
USB_HID_RXSTATUS_t USB_HID_ReceiveData(uint8_t *ptr)
{
  ...
  for(n=0;n<HID_OUT_BUFFER_SIZE+1;n++) { // USB_HID_OUT_BUF[0] is Report ID
    if(n<check) {
